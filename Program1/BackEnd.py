...
class BackEnd:
    ...
    def start(self):
        self.halted = False
        self.running = True
        thread1 = threading.Thread(target= BackEnd.loop, args= (self,self.running))
        thread1.start()
    ...
    def runsinglestep(self):
        i = 0
        thread1 = threading.Thread(target=BackEnd.singleLoop, args=(self, i))
        thread1.start()
    ...
    def runSingleInstruction(self, ins):
        thread1 = threading.Thread(target=BackEnd.singleInstruction, args=(self, ins))
        thread1.start()
